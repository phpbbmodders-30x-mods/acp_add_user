...
<!-- MODX by the phpBB MOD Team XSL file v1.0.1 copyright 2005-2007 the phpBB MOD Team. 
	$Id: modx.prosilver.en.xsl 31 2007-08-02 14:14:45Z Highway of Life $ -->
...
<div id="page-footer">
  <p class="copyright" style="text-align: center; font-size: 10px;" id="lang-foot">MOD UA XSLT File Copyright &#169; 2007 The phpBB Group, this MOD is copyright to the authors listed above.</p>
      ...
      <dd class="mod-about">
	    ...
	    <dt id="lang-d">Description:</dt>
          ...
          <span class="corners-bottom"><span></span></span>
            ...
            <xsl:for-each select="mod:support">
                <xsl:call-template name="give-mod-support"></xsl:call-template>
            </xsl:for-each>
		</dl>
		<xsl:for-each select="mod:author-group">
    ...
    <xsl:template name="give-mod-support">
    	<xsl:if test="count(mod:topic) > 0">
        	<h3 id="lang-ms">MOD Support</h3>
                <div class="mod-about">
                    <div class="inner"><span class="corners-top"><span></span></span>
                            <xsl:for-each select="mod:topic">
                                <xsl:call-template name="give-support-topic"></xsl:call-template>
                            </xsl:for-each>
                        <span class="corners-bottom"><span></span></span>
                    </div>
                </div>
    ...
    <xsl:template name="give-support-topic">
        <dl class="author-info">
            <dt><xsl:value-of select="mod:location" /></dt>
            <dd>
                <a href="{mod:topic-url}">
                    <xsl:value-of select="mod:topic-name" />
                </a>
            </dd>
        </dl>
    </xsl:template>  
